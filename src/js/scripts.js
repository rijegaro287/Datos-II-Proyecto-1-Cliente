...
class tipoDeDato {
    /**
     * Crea una instancia de un tipo de dato primitivo
     * @param {*} valor el valor que contiene el dato
     * @param {*} nombre el nombre del dato
     */
...
class int extends tipoDeDato {
    /**
     * Crea una instancia de un int
     * @constructor
     * @param {int} valor el valor que contiene el dato
     * @param {string} nombre el nombre del dato
     */
    constructor(valor, nombre) {
        super(valor, nombre);
...
class long extends tipoDeDato {
    /**
     * Crea una instancia de un long
     * @constructor
     * @param {long} valor el valor que contiene el dato
     * @param {string} nombre el nombre del dato
     */
    constructor(valor, nombre) {
        super(valor, nombre);
...
class char extends tipoDeDato {
    /**
     * Crea una instancia de un char
     * @constructor
     * @param {char} valor el valor que contiene el dato
     * @param {string} nombre el nombre del dato
     */
    constructor(valor, nombre) {
        super(valor, nombre);
...
class float extends tipoDeDato {
    /**
     * Crea una instancia de un float
     * @constructor
     * @param {float} valor el valor que contiene el dato
     * @param {string} nombre el nombre del dato
     */
    constructor(valor, nombre) {
        super(valor, nombre);
...
/**
 * Representa un double
 */
class double extends tipoDeDato {
    /**
     * Crea una instancia de un double
     * @constructor
     * @param {double} valor el valor que contiene el dato
     * @param {string} nombre el nombre del dato
     */
    constructor(valor, nombre) {
        super(valor, nombre);
...
class reference {
    /**
     * Crea una instancia de una referencia
     * @constructor
     * @param {string} tipoDeReferencia el tipo de dato al que apunta la referencia
     * @param {string} nombre el nombre del dato
     * @param {int} valor el valor que contiene el dato (una dirección de memoria)
     */
...
/**
 * Representa un struct
 */
class struct {
    /**
     * Crea una instancia de un struct
     * @constructor
     * @param {string} nombre el nombre del dato
     * @param {array} variables una lista con las variables que están contenido dentro del struct
     */
 ...
 * @param {string} ruta la ruta del servidor a la que se enviará la información
 * @param {json} mensajeJSON un JSON con los datos que se desea enviar al servidor (puede ser un string)
 * @returns Un Promise con la respuesta del servidor
 */
const post = async(ruta = '', mensajeJSON = {}) => {
...
/**
 * Verifica que haya conexión con el servidor.
 */
...
/**
 * Detiene la ejecución del programa y envía una solicitud al servidor para vaciar la memoria
 */
const detenerEjecucion = async() => {
...
/**
 * Imprime el texto en la sección de STDOut del IDE. Se utiliza para la función print()
 * @param {string} texto el mensaje que se desea imprimir en el STDOut
 */
const imprimirStdOut = async(texto) => {
    let variable;
...
/**
 * Borra el contenido de todas las secciones del IDE
 */
...
/**
 * Se ejecuta cuando se hace click sobre el botón Run
 */
...
/**
 * Se ejecuta cuando se hace click sobre el botón Stop
 */
...
/**
 * Se ejecuta cuando se hace click sobre el botón Next
 */
...
/**
 * Muestra la sección del Log cuando se hace click sobre su nombre
 */
...
/**
 * Muestra la sección del STDOut cuando se hace click sobre su nombre
 */
...
/**
 * Recorre todo el código en busca de errores y crea las variab
 */
const procesadoInicial = async() => {
    while (posicionEnTexto !== editorTextArea.value.length) {
        ...
        if (variable !== undefined) {
            if (variable.hasOwnProperty('variables')) {
                const solicitudCrearStruct = {
                    nombreDeStruct: variable.nombre,
                    variables: variable.variables
                }
                await post('createStruct', solicitudCrearStruct)
                    .then(response => response.text())
                    .then(body => {
                        imprimirLog(`POST enviado. \n Respuesta recibida: ${body}`);
...
/**
 * Recorre el código y separa cada palabra en un array que representa una línea
 * @param {string} texto El texto que se desea procesar (siempre es lo que hay escrito en el IDE)
 * @param {int} posicionInicial La posición en el texto a partir de la que se comenzará el procesado
 * @returns Cada variable nueva que se crea
 */
...
/**
 * Procesa una línea del código y determina la operación que se está realizando en dicha línea
 * @param {array} informacion Un array con todas las palabras de la línea
...
/**
 * Crea una variable de tipo int, long, float o double según sea necesario
 * @param {string} tipoDeVariable El tipo de la variable a crear
 * @param {string} nombreDeVariable El nombre de la variable a crear
 * @param {number} valorDeVariable El valor que guarda la variable
 * @returns Una instancia de la variable creada
 */
const crearNumero = (tipoDeVariable, nombreDeVariable, valorDeVariable) => {
    let variable = null;
...
/**
 * Crea una variable de tipo char
 * @param {string} nombreDeVariable El nombre de la variable a crear
 * @param {char} valorDeVariable El valor que guarda la variable
 * @returns Una instancia de la variable creada
 */
const crearChar = (nombreDeVariable, valorDeVariable) => {
    let variable = null;
...
/**
 * Crea una variable de tipo referencia
 * @param {*} tipoDeVariable El tipo de dato al que apunta la referencia
 * @param {*} nombreDeVariable El nombre de la referencia
 ...
 * @param {array} informacion Un array con la línea donde se declara el struct
 * @param {string} texto El texto escrito en el IDE
 * @param {int} posicionInicial La posición desde la que se terminó de declarar el struct
 * @returns Una instancia del struct creado
 */
...
/**
 * Procesa las variables dentro del struct y las crea
 * @param {string} nombre El nombre del struct
 * @param {string} texto El texto escrito en el IDE
 * @param {int} posicionInicial La posición desde la que se debe comenzar a buscar nuevas variables
 * @returns Una instancia del struct creado
 */
const procesarStruct = async(nombre, texto, posicionInicial) => {
    let palabra = ''
    let informacion = [];
    let operadorIgualEncontrado = false;
    ...
    for (let i = posicionInicial; i < texto.length; i++) {
        ...
        if (caracter === '}') {
            ...
            informacion.push(palabra);
            variablesEnStruct.push(await procesarLinea(informacion));
            posicionEnTexto += 1;
            return;
        } else if (caracter === '=') {
            informacion.push('=');
...
/**
 * Hace una llamada al servidor solicitando la dirección de memoria de una variable
 * @param {string} nombreDeVariable El nombre de la variable cuya dirección de memoria se desea conocer
 * @returns La dirección de memoria de la variable
 */
const solicitarDireccionDeMemoria = async(nombreDeVariable) => {
...
/**
 * Hace una llamada al servidor solicitando el valor al que apunta una referencia
 * @param {string} nombreDePuntero El nombre de la referencia cuyo valor se desea conocer
 * @returns El valor apuntado por la referencia
 */
const solicitarValorApuntado = async(nombreDePuntero) => {
 ...
 * @returns Un promise con la respuesta del servidor
 */
const reasignarVariable = async(informacion) => {
    ...
    if (informacion[0].includes('.')) {
        return await reasignarVariableEnStruct(informacion);
    }
...
/**
 * Cambia el valor de una variable que está dentro de un struct
 * @param {array} informacion Un array con la línea en la que se reasigna la variable 
 * @returns Un promise con la respuesta del servidor
 */
 ...
 * @param {string} texto Las variables involucradas en la operación
 * @param {string} operador La operación que se realiza (suma, resta, etc...)
...
/**
 * Elimina del RAMView las variables creadas dentro de un scope cuando este se cierra
 * @param {array} variablesEliminadas Las variables que estaban dentro del scope
 */
...
/**
 * Actualiza el RAMLiveView cuando se crea una nueva variable
 * @param {*} variable La variable que se desea agregar al RAMLiveView
 */
const actualizarRamLiveView = variable => {
    if (variable.hasOwnProperty('variables')) {
        variable.valor = variable.variables;
        variable.nombre = variable.nombreDeStruct;
        for (let i = 0; i < variable.variables.length; i++) {
            let variableEnStruct = variable.variables[i];
            variable.variables[i].direccionDeMemoria = variableEnStruct.direccion;
            variable.variables[i].nombre = `${variable.nombre}.${variableEnStruct.nombre}`;
        }
    }
    ...
    if (variable.hasOwnProperty('variables')) {
        for (let i = 0; i < variable.variables.length; i++) {
            let variableEnStruct = variable.variables[i];
            actualizarRamLiveView(variableEnStruct);
        }
    }
}

/**
 * Hace una copia del estado actual del RAMLiveView y lo agrega a un array
 */
const clonarEstadoRamLiveView = () => {
    let copiaRamView = [];
...
/**
 * Valida que un dato sea del tipo especificado
 * @param {*} tipoDeDato El tipo de dato esperado
 * @param {*} dato El dato que se desea validar
 * @returns True o False dependiendo de si los tipos coinciden
 */
const validarTipoDeDato = (tipoDeDato, dato) => {
...
/**
 * Remueve todas las ocurrencias de un elemento en un array
 * @param {array} array El array que se desea recorrer
 * @param {*} valor El elemento que se desea eliminar del array
 * @returns El array con todas las ocurrencias del elemento eliminadas
 */
const removerTodasLasOcurrencias = (array, valor) => {
    let i = 0;
 ...
 * @param {array} array El array que se desea convertir
 * @param {boolean} espacios Indica si debe haber espacios entre cada elemento del array
 * @returns El array convertido en string
 */
const arrayToString = (array, espacios = true) => {
    let string = ``;
    if (espacios) {
...
/**
 * Verifica si un dato es un string
 * @param {*} string El dato que se desea validar
 * @returns True o False dependiendo de la validación
 */
